...
In the [getting started with MathML](docs.topic) article, we've already met the `<mfrac>` element to describe a fraction. Let's consider a basic example which adds new elements for roots (`<msqrt>` and `<mroot>`):
...
> **Warning:** Special[math fonts are generally required to make that stretching possible, 
> the previous example relies on web fonts.
> 
{style="warning"}
...
> **Note:** 
> Although the `linethickness` attribute can be used to specify an arbitrary thickness, 
> it is better to keep the default value, which is calculated from parameters specified in the math font.
> 
{style="note"}
